...
void dht11_and_timer_start(uint16_t dht11_measure_period_in_ms)
{
	...
	timer_init(MILLISECONDS, 1300); // To pass the sensor's unstable status, do not send any instruction within 1 seconds after power-up.
	dht_state = 0;
	timer_enable();
}
