...
//	timer_interrupt_triggered_count++; // for debugging
	...
	if (first_unwanted_interrupt_bypassed == 0) {
		...
		if (dht_state == 0)
		{
			timer_reinit_with_new_settings(MICROSECONDS, 65535);
			dht_state = 1;
...
//					measurements_done_count++; // for debugging
					...
					timer_reinit_with_new_settings(MILLISECONDS, dht11_measurement_period_in_ms); // delay between dht11 measurements (IT MUSTN'T SET BELOW 1150MS IN ORDER TO READ PROPERLY,
